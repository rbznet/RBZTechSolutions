function Get-RBZNetworkFindings {
    ...
    if($Config.scan.networkTests){
        try {
            ...
            $dnsOk = $true
            $out.Add((New-RBZFinding -Category 'Network' -Name 'DNS resolution' -Status 'Healthy' -Summary "Resolved $($Config.network.dnsTestName) successfully." -Details "Addresses: $($resolved.IPAddress -join ', ')"))
        } catch {
            $out.Add((New-RBZFinding -Category 'Network' -Name 'DNS resolution' -Status 'Warning' -Summary "Could not resolve $($Config.network.dnsTestName)." -Details $_.Exception.Message -Recommendation 'Check DNS server configuration and network connectivity.'))
        }
        ...
        try {
            ...
            $out.Add((New-RBZFinding -Category 'Network' -Name 'Internet path (TCP 443)' -Status 'Warning' -Summary 'TCP 443 connectivity test failed.' -Details $_.Exception.Message -Recommendation 'Check internet access, firewall, proxy, VPN, or upstream routing.'))
        }
        ...
        try {
            ...
            if($dnsOk -and $tcpOk){
                $out.Add((New-RBZFinding -Category 'Network' -Name 'HTTPS validation' -Status 'Recommend' -Summary 'Internet connectivity is available, but HTTPS validation failed.' -Details $msg -Recommendation 'Review proxy/TLS inspection, trusted root certificates, or local certificate policy. Internet connectivity itself has been confirmed.'))
            } else {
